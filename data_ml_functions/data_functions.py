...
def prepare_data(params):
    ...
    train_excel_data = []  # List to hold the train data for Excel export
    val_excel_data = []
    test_excel_data = []  # List to hold the test data for Excel export
    iteration_count = 0
    max_iterations = 3000
    isTrain = False
    isVal = False
    ...
    for currDir in walkDirs:
        ...
        for root, dirs, files in tqdm(os.walk(os.path.join(params.directories['dataset'], currDir))):
            ...
            if stop_loop:  # Check if we should stop the outer loop as well
                break
            if len(files) > 0:
                slashes = [i for i, ltr in enumerate(root) if ltr == '/' or ltr == '\\']
            ...
            for file in files:
                ...
                if iteration_count > max_iterations:
                    ...
                    stop_loop = True 
                    break  # Exit the loop after reaching 3000 iterations
        
                if file.endswith('_rgb.json'):  # Skip _msrgb images
                    task = partial(_process_file, file, slashes, root, isTrain, isVal, outDir, paramsDict)
    ...
    for future in tqdm(futures):
        ...
        # Collecting data for Excel generation
        for res in file_results:
            # print(res[1])
             # If the file was processed
            
            if res[1] is not None: 
                # print("appending train data")
                entry = {
                    'category': res[1]['category'],
                    'img_path': res[1]['img_path'],
                    'timestamp': res[1]['timestamp']
                }
                if res[3] is True:  # Check if it's validation data
                    val_excel_data.append(entry)
                else:
                    train_excel_data.append(entry)
            ...
            if res[2] is not None: 
                test_excel_data.append({
                    'img_path': res[2]['img_path'],
                    'timestamp': res[2]['timestamp']
                })
    ...
    # Call generate_excel_files to save data to Excel (separate for train and test)
    generate_excel_files(train_excel_data, 'train')
    generate_excel_files(val_excel_data, 'val')
    generate_excel_files(test_excel_data, 'test')


def generate_excel_files(data, data_type):
    """
    Generate an Excel file from a list of dictionaries containing 'category', 'img_path', and 'timestamp'
    ...
    excel_path = f'processed_images_metadata_{data_type}.csv'
    df.to_csv(excel_path, index=False)
    
    # print(f"Excel file for {data_type} created at {excel_path}")

def _process_file(file, slashes, root, isTrain, isVal, outDir, params):
    """
...
#        subImg.save(imgPath)
        ...
        if isTrain or isVal:
            allResults.append((features, {"features_path": featuresPath, "img_path": imgPath, "category": params['category_names'].index(category), "timestamp": timestamp}, None,isVal))
        else:
            allResults.append((None, None, {"features_path": featuresPath, "img_path": imgPath, "timestamp": timestamp},isVal))
...
#     excel_path = 'processed_images_metadata.xlsx'
...
def calculate_class_weights(params):
    ...
    classWeights = class_weight.compute_class_weight(class_weight='balanced', classes=np.unique(ytrain), y=np.array(ytrain))
